...
Here's the proposed phase. It would slot in as Phase 4 (pushing the current GLB Export to Phase 5):

Phase 4 — Textured Viewport
What the user sees
When d4data is configured (File → Set D4Data Path), models render with their actual base color textures instead of the per-submesh colormap. Each submesh displays its resolved diffuse texture mapped via UVs. Submeshes without a decodable texture fall back to the material's base_color_factor as a solid color tint.
┌─────────────────┬────────────────────────────────────────────────┐
│ (model list)    │                                                │
│                 │          3D Viewport (textured)                │
│ ▸ monster_...   │                                                │
│ ▸ monster_...   │        ┌─────────────────────┐                │
│ █ monster_w...  │        │  ████████████████   │                │
│ ▸ player_...    │        │  ██ [Textured] ██   │                │
...
│                 │                                                │
│                 │  ┌──────────────────────────────────────────┐  │
│                 │  │ Verts: 12,480  Tris: 8,320  Mats: 4     │  │
│                 │  │ Textures: 3/4 loaded   [T] Textured ✓   │  │
│                 │  └──────────────────────────────────────────┘  │
├─────────────────┴────────────────────────────────────────────────┤
│  13,247 models.  Filter: ________              [Export GLB ▾]   │
└──────────────────────────────────────────────────────────────────┘
How it works
Phase 3 renders the entire model as one pv.PolyData with a per-submesh scalar colormap. That approach can't map different textures to different submeshes. Phase 4 changes the rendering strategy:

Each submesh becomes its own pv.PolyData added to the plotter separately
Each sub-mesh carries its own UV coordinates (tcoords in VTK)
Each sub-mesh gets either a pv.Texture (decoded base color image) or a solid color= from base_color_factor

Material + texture resolution pipeline
When a model loads and d4data_path is configured:

LoadModelWorker calls load_materials(model_stem, d4data_path) to resolve the material chain
For each material with a BASE_COLOR texture (slot 1 or 19):

Extract the .tex payload from CASC via RustyDemonCLI
Decode via texture_parser (DDS header wrapping → Pillow) → PIL Image
Convert to numpy RGB array for pv.Texture


Worker emits (MeshData, dict[int, PIL.Image], list[Material]) to main thread
Main thread builds one pv.PolyData per submesh, each with its texture or fallback color

Fallback behavior

No d4data configured: render with per-submesh colormap (Phase 3 behavior)
d4data configured but texture extraction fails: use base_color_factor RGB as solid color for that submesh
Static model / no materials: colormap fallback

Toggle
A keyboard shortcut (T key) toggles between textured and colormap rendering modes. The properties overlay shows the current mode: "Textured ✓" or "Untextured". This lets users quickly compare the raw geometry against the textured result.
Phase 4 prompt
The following is a self-contained prompt for a new terminal session. Copy it verbatim.


Project context: d4extract is a Diablo IV 3D model extraction pipeline. It extracts models from D4's CASC archive (via rustydemon, a Rust CLI), parses binary .app files into a MeshData dataclass, resolves materials via load_materials(), decodes textures via texture_parser, and exports to glTF 2.0. We're building D4.Export, a PySide6 + PyVistaQt desktop GUI on top of it.
Phases 1-3 are complete. The GUI at d4extract/src/d4extract/gui/ can:

Phase 1: Pick and validate a D4 game directory, persist via QSettings
Phase 2: Scan CASC and show ~13K models in a filterable list
Phase 3: Click a model → extract from CASC on QThread → parse to MeshData → render in PyVistaQt with per-submesh coloring (solid colormap), orbit camera, wireframe toggle, properties overlay

Phase 3 renders the entire model as ONE pv.PolyData with a per-submesh scalar array and a colormap. This can't apply different textures per submesh. Phase 4 changes the rendering to per-submesh meshes with actual base color textures.
Your task: Build Phase 4 — textured viewport rendering. When d4data_path is configured, resolve each submesh's material chain to find its BASE_COLOR texture, extract and decode the .tex file from CASC, and apply it as a VTK texture on the submesh's geometry using UV coordinates. Fall back to base_color_factor solid color when textures aren't available.
Before writing any code, read these files in order:

All existing GUI code in d4extract/src/d4extract/gui/ — every .py file. Pay special attention to viewport.py (the current load_mesh method) and load_worker.py (the current extraction + parsing pipeline).
d4extract/src/d4extract/formats/material_parser.py — read the load_materials() function signature, what it returns (list of Material objects), and what fields are on each Material (name, textures list of TextureRef, base_color_factor). Understand how TextureRef.role maps to PBR slots (look for "BASE_COLOR").
d4extract/src/d4extract/formats/texture_parser.py — read how .tex payload files are decoded. The key function wraps raw pixels in a DDS header and decodes with Pillow. Understand the input (file path + eTexFormat) and output (PIL Image).
d4extract/src/d4extract/formats/app_parser.py — confirm MeshData.uvs contains TEXCOORD_0 as list[tuple[float, float]] and MeshData.materials can hold resolved Material objects.
d4extract/src/d4extract/casc/rustydemon.py — confirm how to extract a texture file by its CASC path (the TextureRef.path field gives the CASC path like base/meta/Texture/<name>.tex).

Also read these skills:

d4-python-gui — covers PyVistaQt setup and VTK mesh construction patterns
d4-materials-textures — covers the material resolution chain, eShaderTex slot mapping (slot 1 = BASE_COLOR for creatures, slot 19 = BASE_COLOR for characters — check BOTH), texture decode via DDS wrapping, and base_color_factor fallback from rgbavalAvgColor
d4-coordinate-systems — UV convention: D4 and glTF both use V=0 at top (DirectX convention), so no UV flip needed. VTK also expects V=0 at top when using SetTCoords. No conversion required.

What to build:
Modify these files:
gui/
├── widgets/
│   ├── viewport.py            # MODIFY — per-submesh rendering with textures
│   └── properties_bar.py      # MODIFY — show texture mode status
├── workers/
│   └── load_worker.py         # MODIFY — add material resolution + texture decode
├── main_window.py             # MODIFY — wire d4data_path, T-key toggle
└── settings.py                # MODIFY — add d4data_path accessor (if not already present)
load_worker.py modifications:
After parsing MeshData, if d4data_path is configured:

Call load_materials(model_stem, d4data_path) to resolve materials

model_stem is the filename stem from the SNO path (e.g., monster_beast_werewolf)
If this fails or d4data_path is not set, emit MeshData with materials=None (Phase 3 fallback)


For each resolved Material, find its BASE_COLOR TextureRef:

Check tex.role == "BASE_COLOR" in mat.textures
Get its CASC path from tex.path


Extract each unique .tex file from CASC via RustyDemonCLI
Decode each .tex payload via texture_parser → PIL Image
Build a dict mapping material_index → PIL.Image (or material_index → numpy RGB array)
Emit finished signal with (MeshData, texture_dict) — change the signal payload from object to carry both. Use a dataclass or namedtuple:
python@dataclass
class LoadResult:
    mesh_data: MeshData
    textures: dict[int, np.ndarray] | None  # material_index → RGB array (H, W, 3)
    materials: list[Material] | None

Progress updates: "Extracting..." → "Parsing..." → "Resolving materials..." → "Decoding textures (2/4)..." → done
If any texture fails to extract or decode, log a warning and skip it — the viewport will use the fallback color for that submesh

viewport.py modifications:
Replace the single-PolyData rendering with per-submesh rendering:
load_mesh(result: LoadResult) method:

plotter.clear()
Apply coordinate conversion to all positions and normals (same as Phase 3)
For each submesh in mesh_data.submeshes:
a. Slice positions, normals, UVs, and indices for this submesh
b. Rebase indices to start at 0 within the submesh
c. Build a pv.PolyData from the sliced positions + VTK face array
d. Set normals: submesh_mesh.point_data["Normals"] = sliced_normals
e. Set UVs: submesh_mesh.active_texture_coordinates = sliced_uvs
(use submesh_mesh.point_data.set_tcoords("UVs", uv_array) or equivalent PyVista API)
f. If result.textures has an entry for this submesh's material_index:

tex = pv.Texture(result.textures[material_index])
plotter.add_mesh(submesh_mesh, texture=tex, smooth_shading=True)
g. Else if result.materials has a material for this index:
Use material.base_color_factor[:3] as an RGB solid color
plotter.add_mesh(submesh_mesh, color=rgb, smooth_shading=True)
h. Else (no material data at all):
Fall back to colormap index: plotter.add_mesh(submesh_mesh, color=COLORMAP[material_index % len(COLORMAP)], smooth_shading=True)


plotter.reset_camera()

Add a _render_colormap(mesh_data) private method that preserves the Phase 3 rendering path (single PolyData, scalar colormap). This is the fallback and also used for the T-key toggle.
Add toggle_texture_mode():

Store self._textured = True as default when textures are available
Toggle re-renders with either the textured pipeline or the colormap pipeline
Store the current LoadResult so re-rendering doesn't require re-parsing

CRITICAL — PyVista texture API notes:

pv.Texture(numpy_array) accepts an RGB uint8 numpy array of shape (H, W, 3)
Convert PIL Image: np.array(image.convert("RGB")) — ensure uint8, not float
The mesh MUST have texture coordinates set BEFORE adding to the plotter
PyVista uses active_texture_coordinates — set via mesh.active_texture_coordinates = uv_array where uv_array is a numpy float32 array of shape (n_verts, 2)
Each add_mesh() call with texture= applies that texture to that specific mesh. Multiple submesh meshes can each have their own texture.

properties_bar.py modifications:

Add texture mode indicator: "Textured ✓" or "Untextured"
Show texture load count: "Textures: 3/4 loaded" (how many submeshes got textures vs total)
Update update() method to accept LoadResult instead of just MeshData

main_window.py modifications:

Add T-key shortcut → viewport.toggle_texture_mode()
Pass d4data_path from AppSettings to LoadModelWorker constructor
Add menu bar: File → "Set D4Data Path..." if not already present
When d4data_path changes, offer to reload the current model with textures

Exit criteria:

With d4data configured: clicking a model shows it with base color textures applied via UV mapping
Each submesh displays its own texture (different body parts have different textures)
Submeshes without decodable textures show a solid color from base_color_factor
Without d4data configured: models render with the Phase 3 colormap (no regression)
T key toggles between textured and colormap rendering
Properties bar shows "Textured ✓" and texture load count
Progress updates show material resolution and texture decode stages
Failed texture extractions or decodes are handled gracefully (warning logged, fallback color used)
Performance: texture decode runs on the worker thread, only add_mesh calls happen on main thread
Orbit camera, wireframe toggle (W), and frame reset (F) still work in textured mode
